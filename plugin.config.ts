...
export default config => {
  ...
  if (1) {
    ...
    // const outFile = path.join(__dirname, './src/color.less');
  ...
  if (process.env.LY_ENV === 'pro' || process.env.LY_ENV === 'test') {
    config.merge({
      plugin: {
        install: {
  ...
  // // 打包优化 terser-webpack-plugin 配置
  // if (process.env.LY_ENV === 'dev' || process.env.LY_ENV === 'test') {
  //   config.merge({
  //     plugin: {
  //       install: {
